...
QBCore.Commands.Add("palert", "Report the Police", {{name="Message", help="It Police Message"}}, false, function(source, args)
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "judge" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
...
QBCore.Commands.Add("depot", Lang:t("commands.depot"), {{name = "price", help = Lang:t('info.impound_price')}}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "mechanic" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "cardealer" and Player.PlayerData.job.onduty then
...
RegisterNetEvent('police:server:showFingerprintId', function(sessionId)
    ...
    local fid = Player.PlayerData.citizenid
