...
RegisterNetEvent('police:client:SearchPlayerBag', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        QBCore.Functions.TriggerCallback('police:callback:searchbag', function(stashId)
            if stashId then
                ...
                QBCore.Functions.Notify('Bag not Found', 'error', 3000)
            end
        end, playerId)
    else
...
--     if player ~= -1 and distance < 2.5 then
--         local playerId = GetPlayerServerId(player)
--         QBCore.Functions.TriggerCallback('police:callback:searchbag2', function(stashId)
--             if stashId then
--                 TriggerEvent('dpemote:custom:animation', {"warmth"})
--                 TriggerEvent("inventory:client:SetCurrentStash", "storage_bags_"..stashId)
--                 TriggerServerEvent("inventory:server:OpenInventory", "stash", "storage_bags_"..stashId, {
...
--                 TriggerEvent('dpemote:custom:animation', {"c"})
--             else
--                 QBCore.Functions.Notify('Bag not Fount', 'error', 3000)
--             end
--         end, playerId)
--     else
--         QBCore.Functions.Notify("No one nearby!", "error")
...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "mp_arresting", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
